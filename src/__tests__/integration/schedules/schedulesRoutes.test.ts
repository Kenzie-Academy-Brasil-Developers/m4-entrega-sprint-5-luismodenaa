...
describe("/schedules", () => {
  ...
  test("GET /schedules/properties/:id -  must be able to list the schedules of a property", async () => {
    ...
    expect(response.body.schedule[0]).toHaveProperty("id");
    expect(response.body.schedule[0]).toHaveProperty("date");
    expect(response.body.schedule[0]).toHaveProperty("hour");
    expect(response.body.schedule[0]).toHaveProperty("userId");
